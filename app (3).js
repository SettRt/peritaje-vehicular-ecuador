...
/* ══════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════ */
const BRAND_MODELS = [
  { brand:'Vehiculo',   model:'Referencia (foto)', category:'sedan', paint:0xc62828, accent:0x0c0c0c, reference:true },
...
/* ══════════════════════════════════════════════════════════════════
   MODELO DE REFERENCIA (basado en la fotografia proporcionada)
   Cupe/sedan rojo, techo y capo con franja negra, paragolpes
   delantero blanco, bloques amarillos junto a la parrilla negra,
   pilares y zocalo negros, llantas cromadas de 5 rayos.
   ══════════════════════════════════════════════════════════════════ */
function buildReferenceCoupeLP(g) {
  const RED    = 0xc62828;
  const BLACK  = 0x0b0b0c;
  const WHITE  = 0xf2f2f0;
  const YELLOW = 0xe7c93a;

  const P   = M(RED, 0.55, 0.22);
  const BLK = M(BLACK, 0.4, 0.42);
  const WHT = M(WHITE, 0.42, 0.3);
  const YLW = M(YELLOW, 0.55, 0.3);
  const CH  = M(0xd4dde6, 0.85, 0.12);

  /* body shell — low, long-hood, short-deck silhouette like the reference */
  const bPts = [
    {x:-1.74,y:-0.24},{x:1.82,y:-0.24},{x:1.82,y:0.14},
    {x:1.5, y:0.2},   {x:-1.46,y:0.2}, {x:-1.74,y:0.14},
  ];
  g.add(makeCarBody(bPts, 0.74, P));

  /* black roof / pillars — compact, raked greenhouse */
  const cPts = [
    {x:-0.6, y:0.18},{x:0.8, y:0.18},
    {x:0.8,  y:0.62},{x:0.6, y:0.7},
    {x:-0.48,y:0.7}, {x:-0.6,y:0.46},
  ];
  g.add(makeCarBody(cPts, 0.64, BLK));

  /* glass */
  addGlassPane(g, 0.06,0.4, 1.24, 0.74,0.42,0, 0,0,-0.42, 0.5);   // windshield
  addGlassPane(g, 0.06,0.32,1.24, -0.56,0.4,0, 0,0,0.42, 0.42);   // rear window
  addGlassPane(g, 0.88,0.24,0.04,  0.06,0.42, 0.66);              // side L
  addGlassPane(g, 0.88,0.24,0.04,  0.06,0.42,-0.66);              // side R

  /* hood with central black racing stripe */
  box(g, 1.5,0.07,1.5, 1.0,0.18,0, P);
  box(g, 1.5,0.075,0.34, 1.0,0.19,0, BLK);

  /* low black roof cap (per photo, roof reads almost flat black) */
  box(g, 1.16,0.05,1.26, 0.06,0.7,0, BLK);

  /* short trunk deck */
  box(g, 0.46,0.16,1.42, -1.4,0.06,0, P);

  /* black rocker / sill panel along the bottom of the doors */
  box(g, 3.3,0.06,1.5, 0,-0.2,0, BLK);

  /* FRONT — white lower bumper/splitter spanning the nose */
  box(g, 0.24,0.22,1.66, 1.86,-0.18,0, WHT);
  box(g, 0.06,0.04,1.6,  1.94,-0.06,0, CH);

  /* black horizontal band above the bumper, across the nose */
  box(g, 0.08,0.1,1.66, 1.9,0.18,0, BLK);

  /* central black grille block */
  box(g, 0.1,0.18,0.48, 1.86,-0.02,0, BLK);

  /* yellow blocks flanking the grille (turn-signal panels in photo) */
  box(g, 0.12,0.22,0.42, 1.85,0.02, 0.62, YLW);
  box(g, 0.12,0.22,0.42, 1.85,0.02,-0.62, YLW);

  addLight(g,  1.92,0.18, 0.5);
  addLight(g,  1.92,0.18,-0.5);
  addLight(g, -1.66,0.12, 0.5, true);
  addLight(g, -1.66,0.12,-0.5, true);

  /* rear bumper, plain black */
  box(g, 0.12,0.2,1.5, -1.7,-0.16,0, BLK);

  /* side mirrors */
  box(g, 0.1,0.06,0.05, 0.5,0.5, 0.7, BLK);
  box(g, 0.1,0.06,0.05, 0.5,0.5,-0.7, BLK);

  /* chrome 5-spoke wheels, low stance */
  addWheel(g, -0.98, 0.66,0.36, RED);
  addWheel(g, -0.98,-0.66,0.36, RED);
  addWheel(g,  1.04, 0.66,0.36, RED);
  addWheel(g,  1.04,-0.66,0.36, RED);
}
...
function buildBrandVehicle(group, brandModel) {
  ...
  if      (brandModel.reference)  buildReferenceCoupeLP(group);
  else if (category==='pickup')   buildPickupLP(group, paint, accent);
