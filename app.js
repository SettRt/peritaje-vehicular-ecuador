...
function drawFallbackVehicle() {
  ...
  ctx.scale(currentCategory === 'hatchback' ? 0.82 : currentCategory === 'suv' ? 1.05 : 1, 1);

  const bodyColor = currentCategory === 'hatchback' ? '#b3261e' : currentCategory === 'suv' ? '#596579' : '#f8fafc';
  ctx.fillStyle = 'rgba(17, 24, 39, 0.25)';
  ctx.beginPath();
  ctx.ellipse(0, 75, 165, 22, 0, 0, Math.PI * 2);
  ctx.fill();

  ctx.fillStyle = bodyColor;
  roundRect(ctx, -160, -25, 320, 80, 14);
  ctx.fill();
  ...
  roundRect(ctx, currentCategory === 'pickup' ? -80 : -58, -88, 135, 68, 12);
  ctx.fill();

  ctx.fillStyle = '#7fb2e5';
  roundRect(ctx, currentCategory === 'pickup' ? -62 : -42, -78, 84, 34, 7);
  ctx.fill();

  if (currentCategory === 'pickup') {
    ctx.fillStyle = '#c9a646';
    roundRect(ctx, 48, -13, 100, 44, 8);
    ctx.fill();
  }
  ...
  [-98, 98].forEach((x) => {
    ...
    ctx.beginPath();
    ctx.arc(x, 55, 31, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#c7d2de';
    ctx.beginPath();
    ctx.arc(x, 55, 15, 0, Math.PI * 2);
    ctx.fill();
  });
